...
__export(random_teams_exports, {
  RandomCPMTeams: () => RandomCPMTeams,
  cpmSets: () => cpmSets,
  default: () => random_teams_default
});
module.exports = __toCommonJS(random_teams_exports);
var import_teams = __toESM(require("../../random-battles/gen9/teams"));
const cpmSets = {
  Aesap: {
    species: "Aesap",
    ability: "Exhaust",
...
class RandomCPMTeams extends import_teams.default {
  randomCPMTeam(options = {}) {
    ...
    while (pool.length && team.length < this.maxTeamSize) {
      ...
      const cpmSet = this.dex.deepClone(cpmSets[name]);
      if (cpmSet.skip) continue;
      if (!(debug.length || monotype)) {
      ...
      let teraType;
      if (cpmSet.teraType) {
        teraType = cpmSet.teraType === "Any" ? this.sample(this.dex.types.names()) : this.sampleIfArray(cpmSet.teraType);
      }
      const moves = [];
      ...
      const set = {
        ...
        evs: cpmSet.evs ? { ...{ hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0 }, ...cpmSet.evs } : { hp: 84, atk: 84, def: 84, spa: 84, spd: 84, spe: 84 },
        ivs,
...
var random_teams_default = RandomCPMTeams;
