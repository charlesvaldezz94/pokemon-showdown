...
const Moves = {
    ...
    onHit(target) {
      ...
      for (const pokemon of target.side.pokemon) {
        ...
        for (const moveSlot of pokemon.moveSlots) {
          ...
          if (move.flags["noassist"] || this.field.pseudoWeather["gravity"] && move.flags["gravity"] || target.volatiles["healblock"] && move.flags["heal"]) {
            continue;
          }
          moves.push(moveid);
        }
  ...
  copycat: {
    ...
    onHit(pokemon) {
      ...
      if (!move) return;
      if (move.flags["failcopycat"] || this.field.pseudoWeather["gravity"] && move.flags["gravity"] || pokemon.volatiles["healblock"] && move.flags["heal"]) {
        return false;
      }
  ...
  metronome: {
    ...
    onHit(pokemon) {
      const moves = this.dex.moves.all().filter((move) => (![2, 4].includes(this.gen) || !pokemon.moves.includes(move.id)) && (!move.isNonstandard || move.isNonstandard === "Unobtainable") && move.flags["metronome"] && !(this.field.pseudoWeather["gravity"] && move.flags["gravity"]) && !(pokemon.volatiles["healblock"] && move.flags["heal"]));
      let randomMove = "";
      if (moves.length) {
