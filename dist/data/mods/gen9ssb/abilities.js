  ...
  */
  ...
  cascade: {
    ...
    onUpdate(pokemon) {
      ...
      this.add(`c:|${(0, import_scripts.getName)("ausma")}|that's it, yall mfs are about to face the wrath of Big Stall\u2122`);
      this.add(`c:|${(0, import_scripts.getName)("ausma")}|or i guess moreso Big Pult. pick your poison`);
      this.add("-activate", pokemon, "ability: Cascade");
      (0, import_scripts.changeSet)(this, pokemon, import_random_teams.ssbSets["ausma-Mismagius"], true);
      pokemon.cureStatus();
  ...
  blitzofruin: {
    ...
    onStart(pokemon) {
      ...
      this.add("-message", `${pokemon.name}'s Blitz of Ruin lowered the Speed of all surrounding Pok\xE9mon!`);
    },
  ...
  painfulexit: {
    ...
    onBeforeSwitchOut(pokemon) {
      this.add(`c:|${(0, import_scripts.getName)("Breadey")}|Just kidding!! Take this KNUCKLE SANDWICH`);
  ...
  thatshacked: {
    ...
    onResidual(target, source, effect) {
      ...
      this.add(`c:|${(0, import_scripts.getName)((target.illusion || target).name)}|${this.sample(abilMessages)}`);
  ...
  masquerade: {
    ...
    onUpdate(pokemon) {
      if (["mimikyu", "mimikyutotem"].includes(pokemon.species.id) && this.effectState.busted) {
        ...
        this.add(`c:|${(0, import_scripts.getName)("clerica")}|oop`);
      }
    },
  ...
  formchange: {
    ...
    onResidual(pokemon) {
      ...
      if (pokemon.hp > pokemon.maxhp / 2) {
        ...
        this.add(`c:|${(0, import_scripts.getName)("Dawn of Artemis")}|Good, I'm healthy again, time to swap back.`);
      } else {
        if (pokemon.species.id === "necrozmaultra") return;
        this.add(`c:|${(0, import_scripts.getName)("Dawn of Artemis")}|Time for me to transform and you to witness the power of Ares now!`);
        newSet += "-Ultra";
      }
      this.add("-activate", pokemon, "ability: Form Change");
      (0, import_scripts.changeSet)(this, pokemon, import_random_teams.ssbSets[newSet]);
  ...
  hacking: {
    ...
    onStart(pokemon) {
      ...
      this.add(`c:|${(0, import_scripts.getName)(name)}|One moment, please. One does not simply go into battle blind.`);
      const side = pokemon.side.id === "p1" ? "p2" : "p1";
      this.add(
        `message`,
        `ssh sim@pokemonshowdown.com && nc -U logs/repl/sim <<< "Users.get('${this.toID(name)}').popup(battle.sides.get('${side}').pokemon.map(m => Teams.exportSet(m)))"`
      );
      let warnMoves = [];
      let warnBp = 1;
      ...
      if (!warnMoves.length) {
        this.add(`c:|${(0, import_scripts.getName)(name)}|Fascinating. None of your sets have any moves of interest.`);
        return;
      }
      const [warnMoveName, warnTarget] = this.sample(warnMoves);
      this.add(
        "message",
        `${name} hacked into PS and looked at ${name === "Hecate" ? "her" : "their"} opponent's sets. ${warnTarget.name}'s move ${warnMoveName} drew ${name === "Hecate" ? "her" : "their"} eye.`
      );
      this.add(`c:|${(0, import_scripts.getName)(name)}|Interesting. With that in mind, bring it!`);
    },
    flags: {}
  },
  // HoeenHero
  ...
  illiterit: {
    ...
    onTryHit(target, source, move) {
      if (target !== source && move.id.length >= 12) {
        ...
        this.add(`c:|${(0, import_scripts.getName)("in the hills")}|Gee ${source.name}, maybe I should get a dictionary so I can understand what move you just used.`);
        return null;
      }
    },
    flags: { breakable: 1 }
  ...
  anfield: {
    ...
    onStart(target) {
      ...
      for (const pseudoWeather of import_scripts.PSEUDO_WEATHERS) {
  ...
  flashfreeze: {
    ...
    onSourceModifyDamage(damage, source, target, move) {
      if (move.category === "Special" && source.boosts["spa"] > 0 || move.category === "Physical" && source.boosts["atk"] > 0) {
        return this.chainModify(0.75);
      }
    },
    flags: { breakable: 1 }
      ...
      for (const mon of this.getAllActive()) {
        ...
        for (const volatile of volatilesToClear) {
          if (mon.volatiles[volatile]) {
            ...
            if (volatile === "flipped") {
              (0, import_scripts.changeSet)(this, mon, import_random_teams.ssbSets["Clem"]);
              this.add(`c:|${(0, import_scripts.getName)("Clem")}|\u252C\u2500\u2500\u252C\u25E1\uFF89(\xB0 -\xB0\uFF89)`);
            }
  ...
  eoncall: {
    ...
    onAfterMove(source, target, move) {
      if (move.category === "Status" && source.species.baseSpecies === "Latias") {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["MyPearl"], true);
      } else if (move.category === "Special" && source.species.baseSpecies === "Latios") {
        (0, import_scripts.changeSet)(this, source, import_random_teams.ssbSets["MyPearl-Latias"], true);
      }
    },
    flags: {}
  },
    ...
    onDamage(damage, target, source, effect) {
      ...
      if (target.set && !target.set.shiny) {
        if (target.m.damageTaken >= 420) {
          ...
          if (!target.hp) {
            return this.add(`c:|${(0, import_scripts.getName)("PartMan")}|MWAHAHA NOW YOU - oh I'm dead`);
          }
          this.add(`c:|${(0, import_scripts.getName)("PartMan")}|That's it. Get ready to be rapid-fire hugged.`);
          target.clearBoosts();
  ...
  hardcorehustle: {
    ...
    onSourceModifyAccuracy(accuracy, target, source, move) {
      if (this.effectState.fallen) {
        const accMod = [1, 0.95, 0.9, 0.85, 0.8, 0.75];
  ...
  adaptiveengineering: {
    ...
    onResidual(source) {
      if (source?.foes()?.[0] === void 0) return;
  ...
  // Steorra
  ghostlyhallow: {
    shortDesc: "This Pok\xE9mon can hit Normal types with Ghost-type moves.",
    name: "Ghostly Hallow",
  ...
  thegraceofjesuschrist: {
    ...
    onResidual(pokemon) {
      const plates = this.dex.items.all().filter((item2) => item2.onPlate && !item2.zMove);
  ...
  // Valerian
  fullbloom: {
    shortDesc: "This Pok\xE9mon's priority moves have double power.",
    name: "Full Bloom",
    onBasePowerPriority: 30,
    onBasePower(basePower, pokemon, target, move) {
      if (move.priority > 0) {
      ...
      onDamage(damage, pokemon, source, effect) {
        if (damage >= pokemon.hp && pokemon.m.seedActive) {
          if (!pokemon.m.reviverSeedTriggered) {
            ...
            this.add(`c:|${(0, import_scripts.getName)("yeet dab xd")}|An "s"?`);
  ...
  tempochange: {
    ...
    onResidual(pokemon) {
      ...
      if (pokemon.species.name === "Meloetta") {
        (0, import_scripts.changeSet)(this, pokemon, import_random_teams.ssbSets["Zarel-Pirouette"], true);
      } else {
        (0, import_scripts.changeSet)(this, pokemon, import_random_teams.ssbSets["Zarel"], true);
      }
    },
