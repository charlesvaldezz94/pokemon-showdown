...
const Rulesets = {
  ...
  kitakamipokedex: {
    ...
    onValidateSet(set, format) {
      ...
      const species = this.dex.species.get(set.species || set.name);
      if (!kitakamiDex.includes(species.baseSpecies) && !kitakamiDex.includes(species.name) && !this.ruleTable.has("+" + species.id)) {
        return [`${species.baseSpecies} is not in the Kitakami Pok\xE9dex.`];
      }
    }
  },
  blueberrypokedex: {
    ...
    onValidateSet(set, format) {
      ...
      const species = this.dex.species.get(set.species || set.name);
      if (!blueberryDex.includes(species.baseSpecies) && !blueberryDex.includes(species.name) && !this.ruleTable.has("+" + species.id)) {
        return [`${species.baseSpecies} is not in the Blueberry Pok\xE9dex.`];
      }
    }
  },
  potd: {
  ...
  forcemonotype: {
    ...
    onValidateRule(value) {
      ...
      if (type.name === "Stellar") {
        throw new Error(`There are no Stellar-type Pok\xE9mon.`);
      }
      return type.name;
    },
    onValidateSet(set) {
  ...
  teampreview: {
    ...
    onTeamPreview() {
      this.add("clearpoke");
      for (const pokemon of this.getAllPokemon()) {
        let details = pokemon.details.replace(", shiny", "").replace(/(Zacian|Zamazenta)(?!-Crowned)/g, "$1-*");
        if (!this.ruleTable.has("speciesrevealclause")) {
          details = details.replace(/(Greninja|Gourgeist|Pumpkaboo|Xerneas|Silvally|Urshifu|Dudunsparce)(-[a-zA-Z?-]+)?/g, "$1-*");
        }
  ...
  blitz: {
    ...
    ruleset: [
      "Timer Starting = 15",
      "Timer Grace = 30",
      "Timer Add Per Turn = 5",
      "Timer Max Per Turn = 15",
      "Timer Max First Turn = 40"
    ]
  },
  vgctimer: {
    effectType: "Rule",
    ...
    ruleset: [
      "Timer Starting = 420",
      "Timer Grace = 90",
      "Timer Add Per Turn = 0",
      "Timer Max Per Turn = 55",
      "Timer Max First Turn = 90",
      "Timeout Auto Choose",
      "DC Timer Bank"
    ]
  },
  ...
  batonpassstatclause: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        if (!moves.includes("batonpass")) continue;
  ...
  batonpasstrapclause: {
    ...
    onBegin() {
      this.add("rule", "Baton Pass Trap Clause: No Baton Passer may have a way to trap Pok\xE9mon");
    },
  ...
  batonpassstattrapclause: {
    ...
    onBegin() {
      this.add("rule", "Baton Pass Stat Trap Clause: No Baton Passer may have a way to boost stats or trap Pok\xE9mon");
    },
    onValidateTeam(team) {
      ...
      for (const set of team) {
        if (!set.moves.map(this.toID).includes("batonpass")) continue;
        let passableBoosts = false;
        const item = this.toID(set.item);
        const ability = this.toID(set.ability);
        for (const move of set.moves) {
  ...
  stabmonsmovelegality: {
    ...
    checkCanLearn(move, species, setSources, set) {
      ...
      if (!nonstandard && !move.isZ && !move.isMax && !this.ruleTable.isRestricted(`move:${move.id}`)) {
        const speciesTypes = [];
        const moveTypes = [];
        const minObtainableSpeciesGen = this.dex.currentMod === "gen8bdsp" || this.dex.gen === 9 && !this.ruleTable.has("natdexmod") ? this.dex.gen : species.gen;
            ...
            if (baseSpecies.otherFormes) {
              for (const formeName of baseSpecies.otherFormes) {
                if (baseSpecies.prevo) {
                  ...
                  if (prevo2.evos.includes(formeName)) continue;
                }
...
(You have sketched ${moveName} ${count} times.)`);
  ...
  maxtotallevel: {
    ...
    onValidateTeam(team) {
      ...
      if (totalLowestLevels > maxTotalLevel) {
        const thePokemon = pickedTeamSize === team.length ? `all ${team.length} Pok\xE9mon` : `the ${pickedTeamSize} lowest-leveled Pok\xE9mon`;
        return [
  ...
  bonustypemod: {
    ...
    onBegin() {
      this.add("rule", "Bonus Type Mod: Pok\xE9mon have their Tera Type added onto their current ones.");
    },
    onModifySpeciesPriority: 1,
    onModifySpecies(species, target, source, effect) {
      if (!target) return;
  ...
  forceofthefallenmod: {
    ...
    onBegin() {
      this.add("rule", "Force of the Fallen Mod: Pok&\xE9mon pass the move in their last moveslot to their allies when they're KOed");
      for (const pokemon of this.getAllPokemon()) {
  ...
  hackmonsformelegality: {
    ...
    onChangeSet(set, format, setHas, teamHas) {
      let species = this.dex.species.get(set.species);
      if ((species.natDexTier === "Illegal" || species.forme.includes("Totem")) && !["Eevee-Starter", "Floette-Eternal", "Greninja-Ash", "Pikachu-Starter", "Xerneas-Neutral"].includes(species.name) && !this.ruleTable.has(`+pokemon:${species.id}`)) {
  ...
  proteanpalacemod: {
    ...
    onBegin() {
      this.add("rule", "Protean Palace Mod: Pok\xE9mon become the type of the move they use.");
    },
  ...
  illusionlevelmod: {
    ...
    onBegin() {
      this.add("rule", "Illusion Level Mod: Illusion disguises the Pok\xE9mon's true level");
    }
  ...
  uniquepokemoves: {
    ...
    onValidateTeam(team, format, teamHas) {
      const pokemoves = new this.dex.Multiset();
      for (const set of team) {
        if (set.moves?.length) {
          for (const moveid of set.moves) {
      ...
      for (const [moveid, num] of pokemoves) {
        ...
        problems.push(
          `You have ${num} Pok\xE9mon with ${this.dex.species.get(moveid).name} as a Pokemove.`,
          `(Each Pok\xE9mon can only be used as a Pokemove ${uniquePokemoves} time${uniquePokemoves === 1 ? "" : "s"} per team.)`
        );
      }
      return problems;
    }
  ...
  ferventimpersonationmod: {
    ...
    onResidual(pokemon) {
      ...
      const abilitySlot = Object.keys(oldPokemon.abilities).find((x) => oldPokemon.abilities[x] === pokemon.set.ability) || "0";
