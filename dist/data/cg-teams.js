...
if (global.Config && Config.usesqlite && Config.usesqliteleveling) {
  cgtDatabase = (0, import_sim.SQL)(module, { file: "./databases/battlestats.db" });
  void updateLevels(cgtDatabase);
  levelUpdateInterval = setInterval(() => void updateLevels(cgtDatabase), 1e3 * 60 * 60 * 2);
}
class TeamGenerator {
    ...
    for (const i of this.itemPool) {
      if (i.itemUser && !i.isNonstandard) {
        for (const user of i.itemUser) {
          if (import_sim.Dex.species.get(user).requiredItems?.[0] !== i.name) this.specialItems[user] = i.id;
        }
      }
    }
      ...
      if (!movePoolIsTrimmed) {
        if (!isRound2) {
          ...
          for (const moveID2 of movePoolCopy) {
            const move2 = this.dex.moves.get(moveID2);
            if (moves.includes(move2)) continue;
            const weight = this.getMoveWeight(move2, teamStats, species, moves, movesStats, ability, level);
            interimMovePool.push({ move: moveID2, weight });
          }
          interimMovePool.sort((a, b) => b.weight - a.weight);
  ...
  getMoveWeight(move, teamStats, species, movesSoFar, movesStats, ability, level) {
    ...
    if (move.category === "Status") {
      ...
      if (move.id in import_cg_team_data.HARDCODED_MOVE_WEIGHTS) weight2 *= import_cg_team_data.HARDCODED_MOVE_WEIGHTS[move.id];
      ...
      if (move.id === "sleeptalk") {
        if (movesStats.noSleepTalk) weight2 *= 0.1;
        ...
        if (!sleepTalkSpammable) weight2 *= 0.1;
      }
      ...
      if (move.heal && movesStats.healing) weight2 *= 0.5;
      return weight2;
    }
    let basePower = move.basePower;
    if (import_cg_team_data.WEIGHT_BASED_MOVES.includes(move.id) || import_cg_team_data.TARGET_HP_BASED_MOVES.includes(move.id)) basePower = 60;
    ...
    let abilityBonus = (import_cg_team_data.ABILITY_MOVE_BONUSES[this.dex.toID(ability)]?.[move.id] || 1) * (import_cg_team_data.ABILITY_MOVE_TYPE_BONUSES[this.dex.toID(ability)]?.[moveType] || 1);
    ...
    if (move.id in import_cg_team_data.HARDCODED_MOVE_WEIGHTS) weight *= import_cg_team_data.HARDCODED_MOVE_WEIGHTS[move.id];
    ...
    switch (item.id) {
      ...
      case "scopelens":
        ...
        if (moves.some((m) => m.id === "focusenergy")) {
          if (ability === "Super Luck") return 0;
      ...
      case "toxicorb":
        ...
        if (!moves.some((m) => TeamGenerator.moveIsPhysical(m, species) && m.id !== "facade") && !species.types.includes("Fire") && ["Thermal Exchange", "Water Bubble", "Water Veil"].includes(ability)) return 0;
        weight = 0;
