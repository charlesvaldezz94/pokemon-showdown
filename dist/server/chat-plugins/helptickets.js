...
__export(helptickets_exports, {
  BATTLES_REGEX: () => BATTLES_REGEX,
  HelpTicket: () => HelpTicket,
  REPLAY_REGEX: () => REPLAY_REGEX,
  commands: () => commands,
  getBattleLinks: () => getBattleLinks,
  getBattleLog: () => getBattleLog,
  getOpponent: () => getOpponent,
  handlers: () => handlers,
  listOpponentsFrom: () => listOpponentsFrom,
  loginfilter: () => loginfilter,
  notifyStaff: () => notifyStaff,
  pages: () => pages,
  punishmentfilter: () => punishmentfilter,
  settings: () => settings,
  textTickets: () => textTickets,
  tickets: () => tickets,
...
var import_info = require("../chat-commands/info");
var import_config_loader = require("../config-loader");
var import_helptickets_auto = require("./helptickets-auto");
const TICKET_FILE = "config/tickets.json";
const SETTINGS_FILE = "config/chat-plugins/ticket-settings.json";
const TICKET_CACHE_TIME = 24 * 60 * 60 * 1e3;
const TICKET_BAN_DURATION = 48 * 60 * 60 * 1e3;
const BATTLES_REGEX = /\bbattle-(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?/g;
const REPLAY_REGEX = new RegExp(
  `${import_lib.Utils.escapeRegex(Config.routes.replays)}/(?:[a-z0-9]-)?(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?`,
  "g"
);
const REPORT_NAMECOLORS = {
  p1: "DodgerBlue",
  p2: "Crimson",
  p3: "#FBa92C",
  p4: "#228B22",
...
const settings = (() => {
  try {
    return { ...defaults, ...JSON.parse((0, import_lib.FS)(SETTINGS_FILE).readSync()) };
  } catch {
    return { ...defaults };
  }
})();
try {
  const ticketData = JSON.parse((0, import_lib.FS)(TICKET_FILE).readSync());
  for (const t in ticketData) {
    ...
    if (ticket.banned) {
      ...
      if (ticket.created + TICKET_CACHE_TIME <= Date.now()) {
        ...
        if (ticketRoom) {
          const ticketGame = ticketRoom.game;
...
function writeSettings() {
  (0, import_lib.FS)(SETTINGS_FILE).writeUpdate(() => JSON.stringify(settings));
}
...
class HelpTicket extends Rooms.SimpleRoomGame {
  ...
  onLogMessage(message, user) {
    ...
    if ((!user.isStaff || this.ticket.userid === user.id) && (message.length < 3 || blockedMessages.includes(toID(message)))) {
    ...
    if ((!user.isStaff || this.ticket.userid === user.id) && !this.ticket.active) {
      ...
      switch (this.ticket.type) {
        case "PM Harassment":
          this.room.add(
            `|c|~Staff|Global staff might take more than a few minutes to handle your report. If you are being disturbed by another user, you can type \`\`/ignore [username]\`\` in any chat to ignore their messages immediately`
          ).update();
          break;
      }
  ...
  close(result, staff) {
    ...
    tickets[this.ticket.userid] = this.ticket;
    writeTickets();
    this.room.modlog({ action: "TICKETCLOSE", isGlobal: false, loggedBy: staff?.id || "unknown" });
  ...
  static async modlog(entry) {
    await Rooms.Modlog.write("help-texttickets", entry);
  }
  static list(sorter) {
    if (!sorter) {
      sorter = (ticket) => [
    ...
    return import_lib.Utils.sortBy(Object.values(tickets), sorter);
  }
  static logTextResult(ticket) {
    const entry = {
      text: ticket.text,
   ...
   */
  static async getTextLogs(search, date) {
    if (Config.disableripgrep) {
      throw new Chat.ErrorMessage("Helpticket logs are currently disabled.");
    }
    const results = [];
    if (await (0, import_config_loader.checkRipgrepAvailability)()) {
      const searchString = search.length > 1 ? (
        ...
        import_lib.Utils.escapeRegex(JSON.stringify(search[1]).slice(0, -1))
      ) : "";
      const args = [
        `-e`,
        search.length > 1 ? `${search[0]}":${searchString}` : `${search[0]}":`,
        "--no-filename"
      ];
      let lines;
      try {
        lines = await import_lib.ProcessManager.exec([
          `rg`,
          Monitor.logPath(`tickets/${date ? `${date}.jsonl` : ""}`).path,
          ...args
        ]);
      } catch (e) {
  ...
  static uploadReplaysFrom(text, user, conn) {
  ...
  static colorName(id, info) {
    for (const k in info.players) {
      const player = info.players[k];
      if (player === id) {
  ...
  static formatBattleLog(logs, info, reported) {
  ...
  static async visualizeBattleLogs(rooms, reported) {
    ...
    if (existingRooms.length) {
      const chatBuffer = existingRooms.map((room) => this.formatBattleLog(room.log, room, reported)).filter(Boolean).join("");
      if (chatBuffer) {
        return `<div class="infobox"><details class="readmore"><summary><strong>Battle chat logs:</strong><br /></summary>${chatBuffer}</details></div>`;
      }
    }
  }
  ...
  static getTextButton(ticket) {
    let buf = "";
    const titleBuf = [
      ...ticket.text[0].split("\n").map(import_lib.Utils.escapeHTML),
      ...ticket.text[1].split("<br />").map(import_lib.Utils.stripHTML)
    ].slice(0, 3);
    const noteBuf = Object.entries(ticket.notes || {}).map(([userid, note]) => import_lib.Utils.html`${note} (by ${userid})`).join("&#10;");
    ...
    buf += ticket.claimed ? `${ticket.userid}${namelockDisplay}:` : `<strong>${ticket.userid}</strong>${namelockDisplay}:`;
    buf += ` ${ticket.type}</a>`;
    return buf;
  }
  ...
  static getBanMessage(userid, punishment) {
    if (userid !== punishment.id) {
      const { id: punished, reason } = punishment;
      return `You are banned from creating help tickets${punished !== userid ? `, because you have the same IP as ${userid}` : ""}. ${reason ? `Reason: ${reason}` : ""}`;
    }
...
const NOTIFY_ALL_TIMEOUT = 5 * 60 * 1e3;
const NOTIFY_ASSIST_TIMEOUT = 60 * 1e3;
const unclaimedTicketTimer = { upperstaff: null, staff: null };
...
function pokeUnclaimedTicketTimer(hasUnclaimed, hasAssistRequest) {
  ...
  if (hasUnclaimed && !unclaimedTicketTimer[room.roomid]) {
    unclaimedTicketTimer[room.roomid] = setTimeout(
      ...
      hasAssistRequest ? NOTIFY_ASSIST_TIMEOUT : NOTIFY_ALL_TIMEOUT
    );
...
async function listOpponentsFrom(ticket) {
  const opps = new import_lib.Utils.Multiset();
...
async function getBattleLog(battle, noReplay = false) {
  const battleRoom = Rooms.get(battle);
  const seenPokemon = /* @__PURE__ */ new Set();
  let data = null;
  if (battleRoom && battleRoom.type !== "chat" && battleRoom.battle) {
    ...
    if (Rooms.Replays.db) {
      ...
      try {
        const raw = await (0, import_lib.Net)(`https://${Config.routes.replays}/${battle}.json`).get();
        data = JSON.parse(raw);
      } catch {
      }
    }
  ...
  if (data?.log?.length) {
    ...
    for (const line of log) {
      if (line.startsWith("|c|")) {
        ...
        const species = speciesWithGender.split(",")[0].trim();
...
const delayWarnings = {
  ...
  "IP-Appeal": `Please give us all relevant information on how you are connecting to Pok\xE9mon Showdown (if it is through mobile data, at home, a school or work network, etc), and we will review your case as soon as possible.`,
  ...
  other: `If your issue pertains to battle mechanics or is a question about Pok\xE9mon Showdown, you can ask in the <<help>> chatroom.`
};
const ticketTitles = {
...
const textTickets = {
  pmharassment: {
    ...
    async getReviewDisplay(ticket, staff, conn) {
      ...
      const sharedBattles = (0, import_info.getCommonBattles)(ticket.userid, null, reportUserid, null, conn);
    ...
    onSubmit(ticket, text, submitter, conn) {
      ...
      (0, import_info.getCommonBattles)(targetId, Users.get(targetId), submitter.id, submitter, conn);
    }
  },
  inapname: {
    ...
    getReviewDisplay(ticket, staff, conn, state) {
      ...
      const tar = toID(ticket.text[0]);
      const name = import_lib.Utils.escapeHTML(Users.getExact(tar)?.name || tar);
    ...
    async getReviewDisplay(ticket, staff, conn) {
      ...
      for (const link of links) {
        ...
        for (const id of Object.values(roomData.players)) {
          ...
          if (team) {
            const teamNames = team.map((p) => p.name !== p.species ? import_lib.Utils.html`${p.name} (${p.species})` : p.species);
  ...
  ipappeal: {
    ...
    async getReviewDisplay(ticket, staff, conn, state) {
      ...
      let info = state?.ips;
      const stringIps = info?.some((f) => typeof f === "string");
      if (!info || stringIps) {
      ...
      for (const data of info) {
        ...
        if (ipPunishments) {
          const str = ipPunishments.map((p) => `${Punishments.punishmentTypes.get(p.type)?.desc || p.type} as <a href="https://${Config.routes.root}/users/${p.id}">${p.id}</a>${p.reason ? ` (${p.reason})` : ""}`);
    ...
    checker(text, context, pageId, user) {
      ...
      if (punishments.length) {
        for (const [, , punishment] of punishments) {
          if (userids.includes(punishment.id)) {
...
const pages = {
  help: {
    request(query, user, connection) {
      if (!user.named) {
        const buf2 = `>view-help-request${query.length ? "-" + query.join("-") : ""}
|init|html
|title|Request Help
|pagehtml|<div class="pad"><h2>${this.tr`Request help from global staff`}</h2><p>${this.tr`Please <button name="login" class="button">Log In</button> to request help.`}</p></div>`;
      ...
      for (const [i, page] of query.entries()) {
        ...
        switch (page) {
          ...
          case "appeal":
            ...
            if (user.locked || isStaff) {
              ...
              if (!hostfiltered) {
                ...
                for (const type2 of ["timeleft", "reason", "startedit"]) {
                  buf += `<p><Button>${type2}</Button></p>`;
                }
              }
              ...
              if (!hostfiltered || isStaff) {
                for (const type2 of ["public", "homeip", "mobileip", "device"]) {
                  buf += `<p><Button>${type2}</Button></p>`;
                }
          ...
          case "timeleft":
            ...
            break;
          case "reason":
            const punishments = Punishments.search(user.id).map((p) => p[2]).filter((t) => ["LOCK", "NAMELOCK"].includes(t.type));
            ...
            for (const [idx, punishment] of punishments.entries()) {
              if (punishments.indexOf(punishment) !== idx) {
                ...
                buf += import_lib.Utils.html`<p>Your ${punishment.type} was for: ${punishment.reason}.</p>`;
              }
            }
            break;
          case "startedit":
          ...
          default:
            ...
            const submitMeta = import_lib.Utils.splitFirst(meta, "-", 2).join("|");
      ...
      buf = buf.replace(
        /<Button>([a-z]+)<\/Button>/g,
        (match, id) => `<a class="button" href="/view-help-request${curPageLink}-${id}${meta}" target="replace">${this.tr(ticketPages[id])}</a>`
      );
      return buf;
    },
    ...
    tickets(query, user, connection) {
      ...
      for (const ticket of sortedTickets) {
        ...
        buf += `<tr><td>${icon}</td>`;
        buf += `<td>${import_lib.Utils.escapeHTML(ticket.creator)}</td>`;
        buf += `<td>${ticket.type}</td>`;
        buf += import_lib.Utils.html`<td>${ticket.claimed ? ticket.claimed : `-`}</td>`;
        buf += `<td>`;
        ...
        const room = Rooms.get(roomid);
        if (ticket.text) {
          let title = Object.entries(ticket.notes || {}).map(([userid, note]) => import_lib.Utils.html`${note} (by ${userid})`).join("&#10;");
          if (title) {
      ...
      const ticketBans = import_lib.Utils.sortBy(
    ...
    async text(query, user, connection) {
      ...
      buf += await ticketInfo.getReviewDisplay(ticket, user, connection);
      buf += `<br />`;
      buf += `<div class="infobox">`;
      const [text, context] = ticket.text;
      ...
      if (ticket.notes) {
        ...
        for (const staff in ticket.notes) {
          buf += import_lib.Utils.html`<p>${ticket.notes[staff]} (by ${staff})</p>`;
        }
      ...
      if (!ticket.resolved) {
        ...
        buf += import_lib.Utils.html`<strong>Resolved: by ${ticket.resolved.by}</strong><br />`;
        buf += import_lib.Utils.html`<strong>Result:</strong> ${Chat.collapseLineBreaksHTML(ticket.resolved.result)}<br />`;
        if (ticket.resolved.staffReason.includes("PROOF")) {
          buf += import_lib.Utils.html`<strong>Resolver notes:</strong> ${Chat.collapseLineBreaksHTML(ticket.resolved.staffReason)}<br />`;
        }
      }
      return buf;
    },
    ...
    stats(query, user, connection) {
      ...
      if (table === "tickets") {
        ...
        const sortedStats = import_lib.Utils.sortBy(Object.keys(typeStats), (t) => {
          if (col === "type") {
            return t;
        ...
        const sortedStaff = import_lib.Utils.sortBy(Object.keys(staffStats), (staff) => {
          if (col === "staff") {
            return staff;
          }
  ...
  helpticket: {
    ...
    async submit(target, room, user, connection, cmd) {
      ...
      if (ticket?.open || ipTicket) {
        ...
        const helpRoom2 = Rooms.get(`help-${ticket.userid}`);
        if (!helpRoom2) {
          ticket.open = false;
          return;
        }
        helpRoom2.auth.set(user.id, "+");
      ...
      if (textTicket) {
        ...
        void (0, import_helptickets_auto.runPunishments)(ticket, typeId);
...
|deinit`);
      ...
      if (reportTargetType === "user" && reportTarget) {
        switch (ticket.type) {
          case "PM Harassment":
            ...
            break;
          case "Inappropriate Username":
            staffIntroButtons = import_lib.Utils.html`<button class="button" name="send" value="/forcerename ${reportTarget}">Force-rename ${reportTarget}</button> `;
            break;
        }
        staffIntroButtons += import_lib.Utils.html`<button class="button" name="send" value="/modlog room=global, user='${reportTarget}'">Global Modlog for ${reportTarget}</button> <button class="button" name="send" value="/sharedbattles ${user.id}, ${toID(reportTarget)}">Shared battles</button> `;
      }
      if (ticket.type === "Appeal") {
        staffIntroButtons += import_lib.Utils.html`<button class="button" name="send" value="/modlog room=global, user='${user.name}'">Global Modlog for ${user.name}</button>`;
      }
      const introMsg = import_lib.Utils.html`<h2 style="margin:0">${this.tr`Help Ticket`} - ${user.name}</h2>` + `<p><b>${this.tr`Issue`}</b>: ${ticket.type}<br />${this.tr`A Global Staff member will be with you shortly.`}</p>`;
      const staffMessage = [
      ...
      if (reportTargetType === "room") {
        ...
        if (targetID !== ticket.userid) {
          const commonBattles = (0, import_info.getCommonBattles)(
            targetID,
            Users.get(reportTarget),
            ticket.userid,
            Users.get(ticket.userid),
            this.connection
          );
          if (!commonBattles.length) {
            reportTargetInfo += import_lib.Utils.html`There are no common battles between '${reportTarget}' and '${ticket.creator}'.`;
          } else {
            reportTargetInfo += import_lib.Utils.html`Showing ${commonBattles.length} common battle(s) between '${reportTarget}' and '${ticket.creator}': `;
            reportTargetInfo += commonBattles.map(
              (roomid) => import_lib.Utils.html`<a href=/${roomid}>${roomid.replace(/^battle-/, "")}`
            ).join(", ");
          }
        }
      }
      ...
      if (!helpRoom) {
        helpRoom = Rooms.createChatRoom(`help-${user.id}`, `[H] ${user.name}`, {
          isPersonal: true,
          isHelp: true,
          isPrivate: "hidden",
          modjoin: "%",
...
|deinit`);
    ...
    async resolve(target, room, user) {
      this.checkCan("lock");
      const [ticketerName, result] = import_lib.Utils.splitFirst(target, ",").map((i) => i.trim());
      ...
      HelpTicket.logTextResult(ticket);
      notifyStaff();
    ...
    addresponse(target, room, user) {
      this.checkCan("lock");
      const [type, name, response] = import_lib.Utils.splitFirst(target, ",", 2).map((f) => f.trim());
      ...
      if (!(typeId in textTickets)) {
        throw new Chat.ErrorMessage([
          `'${type}' is not a valid text ticket type.`,
      ...
      if (settings.responses[typeId][name] && !this.cmd.includes("f")) {
        throw new Chat.ErrorMessage([
          `That button already exists for that ticket type.`,
    ...
    listresponses(target, room, user) {
      ...
      buf += Object.keys(table).map((type) => `<p>${ticketTitles[type]}<p>` + Object.keys(settings.responses[type]).map((name) => import_lib.Utils.html`<p>- ${name}: "${settings.responses[type][name]}"</p>`).join("")).join("<hr />");
      return this.sendReplyBox(buf);
    },
    listresponseshelp: [
