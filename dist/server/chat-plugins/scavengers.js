    ...
    return new Promise((resolve, reject) => {
      for (const u in this.data) {
        ...
        for (const a in userData) {
          if (/^(?:cumulative|history)-/i.test(a) || a === "name") continue;
        ...
        questions = item.questions.map(
          (q, i) => {
            if (i % 2) {
              ...
              q = q;
              return item.isHTML ? q : import_lib.Utils.escapeHTML(q);
            }
          }
        ).join(" ");
      } else {
...
class ScavengerHunt extends Rooms.RoomGame {
  ...
  loadMod(modData) {
    ...
    if (typeof modData === "string") {
      ...
      if (!import_scavenger_games.ScavMods.twists[modId]) return this.announce(`Invalid mod. Starting the hunt without the mod ${modId}.`);
  ...
  getCreationMessage(newHunt) {
    ...
    return `|raw|<div class="broadcast-blue"><strong>${huntType} scavenger hunt by <em>${hosts}</em> has been started${staffHost}.</strong><div style="border:1px solid #CCC;padding:4px 6px;margin:4px 1px; overflow:scroll; max-height: 50vh"><strong><em>Hint #1:</em> ${this.formatOutput(this.questions[0].hint)}</strong></div>(To answer, use <kbd>/scavenge <em>ANSWER</em></kbd>)</div>`;
  }
  joinGame(user) {
      ...
      if (!this.runEvent("GivePoints")) {
        ...
        if (winPoints || blitzPoints) {
          for (const [i, completed] of this.completed.entries()) {
            if (!completed.blitz && i >= winPoints.length) break;
  ...
  onTallyLeaderboard() {
    ...
    for (const id in this.leftHunt) {
      if (id in this.playerTable) continue;
  ...
  onChatMessage(msg) {
    ...
    const filtered = this.questions.some((q) => q.answer.some((a) => {
      ...
      if (import_lib.Utils.levenshtein(msgId, a, md) <= md) return true;
      return false;
    }));
  ...
  static parseQuestions(questionArray, force = false) {
...
class ScavengerHuntPlayer extends Rooms.RoomGamePlayer {
  ...
  verifyAnswer(value) {
    ...
    return answer.some((a) => sanitizeAnswer(a) === value);
  }
  onNotifyChange(num) {
...
const ScavengerCommands = {
  ...
  create(target, room, user, connection, cmd) {
    ...
    if (!cmd.includes("force") && ["regular", "unrated", "recycled"].includes(gameType) && !mod && room.settings.scavQueue?.length && !room.scavgame) {
  ...
  next(target, room, user) {
    ...
    if (!room.settings.scavQueue[huntId]) return false;
  ...
  settwist(target, room, user) {
    ...
    if (target === "RESET") {
      ...
      if (!import_scavenger_games.ScavMods.twists[twist] || twist === "constructor") throw new Chat.ErrorMessage("Invalid twist.");
  ...
  async huntlogs(target, room, user) {
    ...
    if (!sortingFields.includes(sortMethod)) sortMethod = "points";
  ...
  async playlogs(target, room, user) {
    ...
    if (!sortingFields.includes(sortMethod)) sortMethod = "finish";
