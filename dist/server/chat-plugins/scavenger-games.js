...
class Leaderboard {
  ...
  async htmlLadder() {
    ...
    return `<div class="ladder" style="overflow-y: scroll; max-height: 170px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Points</th></tr>${data.map((line) => `<tr><td>${line.rank}</td><td>${line.name}</td><td>${line.points}</td></tr>`).join("")}</table></div>`;
  }
}
const TWISTS = {
  perfectscore: {
  ...
  timetrial: {
    ...
    onComplete(player, time, blitz) {
      ...
      import_lib.Utils.sortBy(this.completed, (entry) => entry.duration);
      player.destroy();
      return true;
    }
  },
  scavengersfeud: {
    ...
    onSubmit(player, jumble, value) {
      ...
      if (currentQuestion + 1 === this.questions.length) {
        this.guesses[player.id] = value.split(",").map((part) => (0, import_scavengers.sanitizeAnswer)(part));
        this.onComplete(player);
        return true;
      }
    },
