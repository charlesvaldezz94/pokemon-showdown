...
function parseCredits(amount) {
  ...
  if (Math.abs(credits) < 500) credits *= 1e3;
...
class Auction extends Rooms.SimpleRoomGame {
  ...
  setMinBid(amount) {
    ...
    if (amount > 5e5) throw new Chat.ErrorMessage(`The minimum bid must not exceed 500,000.`);
    this.minBid = amount;
  }
