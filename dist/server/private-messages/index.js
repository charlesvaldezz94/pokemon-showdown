...
const PrivateMessages = new class {
  ...
  async sendReceived(user) {
    ...
    for (const { message, time, sender } of messages) {
      user.send(
        `|pm|${this.getIdentity(sender)}|${this.getIdentity(user)}|/html ${import_lib.Utils.escapeHTML(message)} <i>[sent offline, <time>${new Date(time).toISOString()}</time>]</i>`
      );
    }
  }
  getIdentity(user) {
  ...
  clearSeen() {
    return PM.run(import_database.statements.clearSeen, [Date.now(), SEEN_EXPIRY_TIME]);
  }
  ...
  clearOffline() {
    return PM.run(import_database.statements.clearDated, [Date.now(), EXPIRY_TIME]);
  }
  destroy() {
    void PM.destroy();
  }
...
if (import_config_loader.Config.usesqlite) {
  if (!process.send) {
    ...
    void PM.run(import_database.statements.clearDated, [Date.now(), EXPIRY_TIME]);
