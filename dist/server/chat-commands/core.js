...
const crqHandlers = {
  userdetails(target, user, trustable) {
    ...
    return {
      ...
      group,
      customgroup: sectionleader ? "Section Leader" : void 0,
      autoconfirmed: targetUser.autoconfirmed ? true : void 0,
      status: targetUser.getStatus() || void 0,
      rooms: roomList,
      friended: user.friends?.has(targetUser.id) || void 0
    };
  },
  ...
  roominfo(target, user, trustable) {
    ...
    if (!targetRoom || targetRoom.settings.isPrivate && !user.inRooms.has(targetRoom.roomid) && !user.games.has(targetRoom.roomid)) {
      const roominfo2 = { id: target, error: "not found or access denied" };
      return roominfo2;
    }
    let visibility;
    if (targetRoom.settings.isPrivate) {
...
const commands = {
  ...
  async msgroom(target, room, user, connection) {
    const [targetId, message] = import_lib.Utils.splitFirst(target, ",").map((i) => i.trim());
    if (!targetId || !message) {
  ...
  async offlinemsg(target, room, user) {
    ...
    let [username, message] = import_lib.Utils.splitFirst(target, ",").map((i) => i.trim());
  ...
  invite(target, room, user) {
    ...
    if (!pmTarget) {
      ...
      for (const toInvite of users) {
        this.parse(`/pm ${toInvite}, /invite ${targetRoom2}`);
      }
      return;
    }
  ...
  async rank(target, room, user) {
    ...
    buffer += import_lib.Utils.html`<tr><td colspan="8">User: <strong>${target}</strong></td></tr>`;
  ...
  hiderank(target, room, user, connection, cmd) {
    ...
    const group = isShow ? Users.globalAuth.get(user.id) : target.trim() || Users.Auth.defaultSymbol();
  ...
  language(target, room, user) {
    if (!target) {
      const language2 = Chat.languages.get(user.language || "english");
      return this.sendReply(this.tr`Currently, you're viewing Pokémon Showdown in ${language2}.`);
    }
  ...
  languagehelp: [
    ...
    `/language [language] - Changes the language Pok\xE9mon Showdown will be displayed to you in.`,
...
- ${result.slice(1).replace(/\n/g, "\n- ")}`);
  ...
  help(target, room, user) {
    ...
    const getHelp = (namespace, cmds) => {
      ...
      const curHandler = namespace[cmd];
      ...
      if (isPrivate && !user.can(requiredPerm)) {
