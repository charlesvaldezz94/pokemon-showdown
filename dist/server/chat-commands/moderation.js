...
` + import_lib.Utils.sortBy(names).map((userid) => {
  ...
  sectionleaders(target, room, user, connection) {
    ...
    for (const [id, username] of usernames) {
      ...
      if (!sectionid2) continue;
      if (!sections[sectionid2]) sections[sectionid2] = /* @__PURE__ */ new Set();
...
`));
  ...
  ban(target, room, user, connection, cmd) {
    ...
    if (targetUser.id in room.users || user.can("lock")) {
      targetUser.popup(
        `|modal||html|<p>${import_lib.Utils.escapeHTML(user.name)} has banned you from the room ${room.roomid} ${room.subRooms ? ` and its subrooms` : ``}${week ? " for 1 week" : " for 2 days"}.</p>${publicReason ? `<p>Reason: ${import_lib.Utils.escapeHTML(publicReason)}</p>` : ``}<p>To appeal the ban, PM the staff member that banned you${room.persist ? ` or a room owner. </p><p><button name="send" value="/roomauth ${room.roomid}">List Room Staff</button></p>` : `.</p>`}`
      );
    }
    this.addModAction(
      `${name} was banned${week ? " for 1 week" : " for 2 days"} from ${room.title} by ${user.name}.${publicReason ? ` (${publicReason})` : ``}`
    );
...
Your ban will expire in a few days.`
  ...
  deroomvoiceall(target, room, user) {
    ...
    if (!target) {
      ...
      throw new Chat.ErrorMessage([
        "THIS WILL DEROOMVOICE ALL ROOMVOICED USERS.",
  ...
  promote(target, room, user, connection, cmd) {
    ...
    if (!cmd.startsWith("global")) {
      ...
      if (!groupid && nextGroup === Users.Auth.defaultSymbol()) groupid = "deauth";
  ...
  declare(target, room, user) {
    ...
    this.checkChat();
    if (target.length > 2e3) throw new Chat.ErrorMessage("Declares should not exceed 2000 characters.");
  ...
  unblacklistall(target, room, user) {
    ...
    if (!target) {
      ...
      throw new Chat.ErrorMessage([
        "THIS WILL UNBLACKLIST ALL BLACKLISTED USERS IN THIS ROOM.",
