...
class Battle {
  ...
  fieldEvent(eventid, targets) {
    ...
    while (handlers.length) {
      ...
      if (handler.state?.target instanceof import_pokemon.Pokemon) {
      ...
      } else if (handler.state?.target instanceof import_side.Side && !handler.state.isSlotCondition) {
  ...
  singleEvent(eventid, effect, state, target, source, sourceEffect, relayVar, customCallback) {
    ...
    if (callback === void 0) return relayVar;
  ...
  runEvent(eventid, target, source, sourceEffect, relayVar, onEffect, fastExit) {
    ...
    if (source instanceof import_pokemon.Pokemon) effectSource = source;
    ...
    for (const handler of handlers) {
      if (handler.index !== void 0) {
        if (!targetRelayVars[handler.index] && !(targetRelayVars[handler.index] === 0 && eventid === "DamagingHit")) continue;
        if (handler.target) {
  ...
  getCallback(target, effect, callbackName) {
    ...
    if (callback === void 0 && target instanceof import_pokemon.Pokemon && this.gen >= 5 && callbackName === "onSwitchIn" && !effect.onAnySwitchIn && (["Ability", "Item"].includes(effect.effectType) || // Innate abilities/items
  ...
  getTeam(options) {
    ...
    if (typeof team === "string") team = import_teams.Teams.unpack(team);
    if (team) return team;
    if (!options.seed) {
